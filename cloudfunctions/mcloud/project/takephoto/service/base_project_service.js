...
class BaseProjectService extends BaseService {
	...
	async initSetup() {
		...
		let F = (c) => 'bx_' + c;
		const INSTALL_CL = 'setup_takepotho';


		const COLLECTIONS = ['setup', 'admin', 'log', 'news', 'activity', 'activity_join', 'fav', 'user', 'score', 'goods', 'order'];
		const CONST_PIC = '/images/cover.gif';



		const NEWS_CATE = '1=公告通知,2=项目总结';
		const ACTIVITY_CATE = '1=交通秩序,2=市容环境,3=公共秩序,4=其他任务';
