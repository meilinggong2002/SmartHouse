...
Page({
	...
	_delActivity: async function (e, that) {
		...
		pageHelper.showConfirm('确认删除？删除后上报数据将一并删除且不可恢复', callback);
	},
	...
	_getSearchMenu: function () {
		...
		let sortItem2 = [
			...
			{ label: '按上报人数', type: 'sort', value: 'ACTIVITY_JOIN_CNT|desc' },
			{ label: '按开始时间', type: 'sort', value: 'ACTIVITY_START|desc' },
			{ label: '按开始时间', type: 'sort', value: 'ACTIVITY_START|desc' },
			{ label: '按截止时间', type: 'sort', value: 'ACTIVITY_STOP|desc' },
		];
	...
	bindMoreTap: async function (e) {
		...
		let orderDesc = (order == 0) ? '撤销置顶' : '置顶';

		let vouch = this.data.dataList.list[idx].ACTIVITY_VOUCH;
		let vouchDesc = (vouch == 0) ? '推荐到首页' : '撤销首页推荐';
	...
	bindJoinMoreTap: async function (e) {
		let itemList = ['上报记录管理', '导出记录Excel表格'];
