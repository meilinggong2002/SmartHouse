...
Component({
	...
	methods: {
		...
		_getList: async function (page) {
			...
			// 单日期 
			if ((this.data.doDateOne) && this.data.startDate) {
				params.search = this.data.startDate + '#';
			}
		...
		bindDateSearchTap: function (e) {
			...
			else if (this.data.doDateOne) {
				if (!this.data.startDate.includes('-')) return pageHelper.showNoneToast('请选择日期');
			}
